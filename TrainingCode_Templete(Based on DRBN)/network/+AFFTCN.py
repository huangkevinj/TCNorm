...
from models.network.AffineTCN import InvBlock
...
class DRBN_BU(nn.Module):
    ...
    def part_forward(self, x):
        ...
        if flag == 2:
            ...
            f_s2_norm = self.norm1(self.RDBs[0](f_s1))
            f_s2 = self.Down1(f_s2_norm)
            f_s4_norm = self.norm2(self.RDBs[1](f_s2))
            f_s4 = self.Down2(f_s4_norm)
        else:
            f_first = self.Relu(self.SFENet1(input_x))
            f_s1 = self.Relu(self.SFENet2(f_first))
